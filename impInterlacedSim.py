...
class Biofilm:
    def __init__(self, sizeDomain = (0.2,0.2), meshSize = (24,24), numPP = 10, numS1 = 10, diffDt = 0.1):
        ...
        self.ppDivisions = []
        self.s1Divisions = []
        spacingPP = sizeDomain[0] / (numPP+3.0)
        spacingS1 = sizeDomain[0] / (numS1+3.0)
        for i in range(numPP):
            radius = 0.001   # mm (1 micron)
            mass = .28  # picograms
            pheno = 'pp'
            x = i * spacingPP + spacingPP
    ...
    def clone(self, t):
        for b in self.bugList:
            if b.divide() == True:
                ...
                y = b.location[1] + (1.0 + random.random())*b.radius*2.0
